...
function Game(screen){
    ...
    that.compute_size = function(){
        ...
        var xy = findPos(that.screen); x = xy[0];var y = xy[1];
    ...
    that.register_events = function(){
        ...
        document.body.onkeydown = that.event;
...
function JsSnake(screen,config){
    ...
    that.clear = function(){
        (function(){var r3tr=[];for (var i=0;i<that.objects.length;i++){var obj=that.objects[i];if (true){ r3tr.push(obj.remove()); }};return r3tr;})();
    }
    ...
    that.check_pos = function(pos){
        for (var a=0;a<that.apples.length;a++){
            var a=that.apples[a];
            if (a.collide(pos))return false;
        }
        for (var d=0;d<that.deaths.length;d++){
            var d=that.deaths[d];
            if (d.collide(pos))return false;
        }
        for (var m=0;m<that.mongeese.length;m++){
            var m=that.mongeese[m];
            if (m.collide(pos))return false;
        }
        for (var s=0;s<that.snake.length;s++){
            var s=that.snake.segs[s];
            if (s.collide(pos))return false;
        }
    ...
    that.step = function(){
        if (that.paused || !that.running)return;
        //alert([].map)
        //alert(that.objects.map)
        //that.objects.map(alert);
        that.objects.map(function(x){x.step();});
...
function Sprite(parent,img,pos){
    ...
    that.update = function(){
        var xy = that.pos; x = xy[0];var y = xy[1];
        var ab = findPos(parent.screen)
; a = ab[0];var b = ab[1];        setPos(that,[parent.size["by"]*x+a,parent.size["by"]*y+b]);
    }
    that.step = function(){};
    that.event = function(){};
    that.src = img;
...
function Mongoose(parent){
    ...
    that.step = function(){
        ...
        var xy = parent.snake.pos; x = xy[0];var y = xy[1];
        var ab = that.pos; a = ab[0];var b = ab[1];
...
function Snake(parent,pos,num_segs){
    ...
    that.remove = function(){
        (function(){var r3tr=[];for (var i=0;i<that.segs.length;i++){var seg=that.segs[i];if (true){ r3tr.push(seg.remove()); }};return r3tr;})();
        that._remove();
    }
